...
class MovePecas {
    ...
    public MovePecas(Jogador[] listaDeJogadores, int gameMode) {
    	...
    	initializeCards();
    	ordemDeJogada(listaDeJogadores);
  ...
  //Inicializa o deck de cartas dinamicas
  	...
  	//Embaralha o deck de cartas dinamicas
  	public void shuffle() {
  		Collections.shuffle(this.deckCartaDinamica);
  	}
    ...
    //Define a ordem de jogada dos jogadores
    public void ordemDeJogada(Jogador[] listaDeJogadores) {
    	//Pega valores únicos dos dados jogados
    	final int tam = listaDeJogadores.length;
    	Set<Integer> uniqueDados = new LinkedHashSet<Integer>(tam);
    	ArrayList<Integer> dados = new ArrayList<Integer>(tam);
    	...
    	for (int j=0; j<tam; j++) {
    		...
    		jogadoresOrdenados.add(listaDeJogadores[j]);
    	}
